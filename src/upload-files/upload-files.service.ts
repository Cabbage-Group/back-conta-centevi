...
export class UploadFilesService {
  async agrupamientoExcel(filePath: string): Promise<string> {
    ...
    data = data.map((row) => {
      ...
      if (row['Fecha']) {
        row['Fecha'] = excelDateToJSDate(row['Fecha'])
          .toISOString()
          .split('T')[0];
      }
    ...
    const finalGroupedData = _(groupedData)
      .groupBy('Cuenta')
      .map((rows, account) => {
        const cambioPeriodo = rows.filter(
          (row) => row.DESC === 'Cambio de Periodo Corriente',
        );
        const balanceFinal = rows.filter((row) => row.DESC === 'Balance Final');
        const otherRows = rows.filter(
          (row) =>
            row.DESC !== 'Cambio de Periodo Corriente' &&
            row.DESC !== 'Balance Final',
        );
        return [...otherRows, ...cambioPeriodo, ...balanceFinal, {}];
      })
      .flatten()
      .value();
    ...
    sortedData.forEach((dat, index) => {
      ...
      if (
        dat['Cuenta'] == undefined &&
        dat['Fecha'] == undefined &&
        dat['Referencia'] == undefined &&
        dat['Source'] == undefined &&
        dat['Descripción'] == undefined &&
        dat['Debito'] == undefined &&
        dat['Credito'] == undefined &&
        dat['Balance'] == undefined
      ) {
        inicioGrupo = false;
      }
    ...
    const outputFilePath = path.join(
      process.cwd(),
      'uploads',
      `archivo_agrupado_${Date.now()}.xlsx`,
    );
  ...
  async agrupamientoExcelAnterior(filePath: string): Promise<string> {
    const workbook = xlsx.readFile(filePath);
    const sheetName = workbook.SheetNames[0];
    const sheet = workbook.Sheets[sheetName];
