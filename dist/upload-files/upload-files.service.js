...
let UploadFilesService = class UploadFilesService {
    async agrupamientoExcel(filePath) {
        ...
        data = data.map((row) => {
            ...
            if (row['Fecha']) {
                row['Fecha'] = (0, recursos_1.excelDateToJSDate)(row['Fecha'])
                    .toISOString()
                    .split('T')[0];
            }
            return row;
        ...
        const finalGroupedData = _(groupedData)
            ...
            const otherRows = rows.filter((row) => row.DESC !== 'Cambio de Periodo Corriente' &&
    ...
    async agrupamientoExcelAnterior(filePath) {
        const workbook = xlsx.readFile(filePath);
        const sheetName = workbook.SheetNames[0];
        const sheet = workbook.Sheets[sheetName];
        let data = xlsx.utils.sheet_to_json(sheet, { defval: '' });
